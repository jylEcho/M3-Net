...
class Model(nn.Module):
    ...
    def forward(self, img32, img64, img96):
        ...
        tokens = torch.cat([f_small, f_cross], dim=1)
...
if __name__ == "__main__":
    ...
    class DummyNet(nn.Module):
        ...
        def forward(self, x):
            # 假设输入 [B, 3, H, W]
            B = x.shape[0]
            return self.fc(torch.randn(B, 128, device=x.device))

    # ===== 实例化主模型 =====
    net32 = DummyNet()
    net64 = DummyNet()
    net96 = DummyNet()
    model = Model(net32, net64, net96, freeze_backbones=True).cuda()

    # ===== 构造输入样本 =====
    dummy_32 = torch.randn(1, 1, 32, 32, 32).cuda()
    dummy_64 = torch.randn(1, 1, 64, 64, 64).cuda()
    dummy_96 = torch.randn(1, 1, 96, 96, 96).cuda()

    # ===== 计算 FLOPs 和参数 =====
    flops, params = profile(model, inputs=(dummy_32, dummy_64, dummy_96))
